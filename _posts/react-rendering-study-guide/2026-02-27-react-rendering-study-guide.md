...
- "렌더됐다"와 "DOM이 바뀌었다"를 같은 의미로 생각하면 디버깅이 바로 꼬인다.
- props가 안 바뀌었는데 자식이 다시 실행되는 이유는 대부분 부모 리렌더링이었다.
- `React.memo`, `useMemo`, `useCallback`은 시작점이 아니라 원인을 찾은 뒤 마지막에 넣는 도구라는 점을 다시 확인했다.
...
이번에 다시 정리하면서도 렌더 횟수 자체보다 원인을 말로 설명할 수 있는지가 더 중요한 기준이라는 점이 선명해졌습니다.
