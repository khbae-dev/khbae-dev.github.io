...
이번 글은 `concurrently`를 설치하지 않고 `npx`로 바로 실행하는 방법을 중심으로 정리한 스터디 리포트입니다.
...
이 방식은 `concurrently`가 프로젝트에 설치되어 있어야 합니다.
...
- `concurrently는 내부 또는 외부 명령, 실행할 수 있는 프로그램, 또는 배치 파일이 아닙니다.`
- `npm install` 도중 `peer dependency conflict`
- Nx, Vite, 기타 도구 버전 충돌
- `node_modules`가 이미 복잡한 상태라 추가 설치가 부담됨
...
중요한 점은 프로젝트의 `package.json`이나 `node_modules`에 직접 의존성을 추가하지 않는다는 것입니다.
...
- 네트워크 제한이 있는 환경에서는 처음 실행이 막힐 수 있다
...
필요하다면 다음에는 `concurrently` 옵션(`-k`, `-n`, `-c`)까지 포함해서 로그 보기 좋게 구성하는 방식도 정리해볼 수 있겠습니다.
